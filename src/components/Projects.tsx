...
const Projects = () => {
  ...
  return (
    <section
      ref={sectionRef}
      id="projects"
      className="py-20 px-8 max-w-7xl mx-auto lg:pl-32 lg:pr-32"
    >
      <motion.div
              ...
              <div className="relative grid md:grid-cols-12 gap-4 md:gap-8">
                ...
                <div className={`md:col-span-5 ${index % 2 === 0 ? 'md:order-2' : 'md:order-1'}`}>
                  <div className="space-y-4">
                    ...
                    <p className="text-sm font-mono text-[rgb(var(--text-secondary))]">
                      {project.duration}
                    </p>
                    ...
                    <ul className="space-y-2">
                      {project.points.map((point, idx) => (
                        <li
                          key={idx}
                          className="text-[rgb(var(--text-secondary))] text-sm flex items-start"
                        >
                    ...
                    <div className="flex flex-wrap gap-3 pt-2">
                      {project.tech.map(tech => (
                        <span key={tech} className="text-[rgb(var(--primary))] text-sm font-mono">
                          {tech}
                        </span>
                      ))}
                    </div>
              ...
              <div className="relative grid md:grid-cols-12 gap-4 md:gap-8 bg-[rgba(var(--primary),0.05)] rounded-lg p-6 hover:bg-[rgba(var(--primary),0.1)] transition-all duration-300">
                <div className="md:col-span-4 flex items-center justify-center">
                  <div className="relative w-32 h-32 group-hover:scale-110 transition-transform duration-300">
                    <svg
                      className="w-full h-full text-[rgb(var(--primary))]"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                ...
                <div className="md:col-span-8">
                  <div className="space-y-4">
                    ...
                    <p className="text-lg text-[rgb(var(--text-secondary))] group-hover:text-[rgb(var(--text-primary))] transition-colors">
                      Explore all my work on GitHub, featuring various projects from small utilities
                      to fullstack and open source applications.
                    </p>
                  </div>
                </div>
              </div>
